...
**Execution order:** #1 (done) → #3 (done) → #4 (done) → #5 (done) → #6 (done) → #7 (done) → #8 (done) → #9 (done) → #10 (done).
#2 left the queue 2026-07-12 — decomposed into the sibling `curation-policy` epic (its DList story
inherits the `EVENT_TAGS` socket #9 shaped for it). **Every story in this epic is now Done**; the
epic stays Active per PO direction (2026-07-12) while its sibling `curation-policy` is in flight —
epic retirement and book close are deferred decisions, not defaults.
