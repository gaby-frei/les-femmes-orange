...
- #10 — `10-feed-header-semantics` — **(to be planned via /plan-feature)** What "X members contributing" counts in the two-provider era: distinct authors (status quo, deliberately unfiltered since #8 — non-member Provider-2 authors inflate it), members-only, endorsers, or a reworded subtitle. The one piece of superseded #2 that stays in this epic (PO decision 2026-07-12). *(Not started — no story file yet)*

**Execution order:** #1 (done) → #3 (done) → #4 (done) → #5 (done) → #6 (done) → #7 (done) → #8 (done) → #9 (done) → **#10** (to plan).
#2 left the queue 2026-07-12 — decomposed into the sibling `curation-policy` epic (its DList story
inherits the `EVENT_TAGS` socket #9 shaped for it). This epic's remaining work is exactly #10
(header semantics). Both epics are Active in the shared `community-feed` book; neither is closing yet.
