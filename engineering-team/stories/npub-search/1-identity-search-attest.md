...
- [ ] **No profile found → still vouchable, with a warning.** *(Amended 2026-07-31 — PO rollback of the original O3 view-only rule + PO warning copy.)* Given a valid identity with no discoverable profile metadata, the candidate still appears — its short npub shown in the name slot — and can be vouched **like any other valid npub**, with this verbatim warning beneath: "This profile is missing visible profile metadata. Double check that you know and trust the owner of this pubkey."
