...
- Topic detection: **hashtag-only** (`t` tags). **Qualifying hashtag list (v1):** `nostr`, `asknostr`, `grownostr`, `bitcoin`, `btc`, `lightning`, `sats`. A note qualifies if it carries **any** of these as a `t` tag.
- Feed size cap: **100**.
